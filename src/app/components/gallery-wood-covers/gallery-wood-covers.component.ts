import { Component, OnInit } from '@angular/core';

@Component({
  selector: 'app-gallery-wood-covers',
  templateUrl: './gallery-wood-covers.component.html',
  styles: []
})
export class GalleryWoodCoversComponent implements OnInit {

	constructor() { }

	ngOnInit() {
	}

  imgArr = [
    {'src': './assets/img/gallery/wood-covers/wood-covers1.jpg'},
    {'src': './assets/img/gallery/wood-covers/wood-covers2.jpg'},
    {'src': './assets/img/gallery/wood-covers/wood-covers3.jpg'},
    {'src': './assets/img/gallery/wood-covers/wood-covers4.jpg'},
    {'src': './assets/img/gallery/wood-covers/wood-covers5.jpg'},
    {'src': './assets/img/gallery/wood-covers/wood-covers6.jpg'},
    {'src': './assets/img/gallery/wood-covers/wood-covers7.jpg'},
    {'src': './assets/img/gallery/wood-covers/wood-covers8.jpg'},
    {'src': './assets/img/gallery/wood-covers/wood-covers9.jpg'},
    {'src': './assets/img/gallery/wood-covers/wood-covers10.jpg'},
    {'src': './assets/img/gallery/wood-covers/wood-covers11.jpg'},
    {'src': './assets/img/gallery/wood-covers/wood-covers12.jpg'}
   ];
}
